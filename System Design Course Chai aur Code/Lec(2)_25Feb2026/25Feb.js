...
// 17. Mutex
// A mutex (short for "mutual exclusion") is a synchronization primitive used in concurrent programming to prevent multiple threads or processes from accessing a shared resource simultaneously.
// It ensures that only one thread or process can hold the mutex at a time, allowing for safe access to shared data and preventing race conditions.
...
// Distributed mutexes can be implemented using consensus algorithms, leader election, or distributed coordination services like Apache ZooKeeper or etcd.
...
// It provides a simple API for reading and writing key-value pairs, as well as features like watch notifications, transactions, and leases for managing the lifecycle of keys.
// etcd is commonly used in container orchestration platforms like Kubernetes to store cluster state, configuration data, and service discovery information.

// 19. Apache ZooKeeper
// Apache ZooKeeper is a distributed coordination service that provides a centralized infrastructure for managing configuration, synchronization, and naming in distributed systems.
...
// ZooKeeper provides a simple API for reading and writing data, as well as features like watches, ephemeral nodes, and leader election for managing the state of distributed systems.
// ZooKeeper is commonly used in distributed applications for tasks such as configuration management, service discovery, leader election, and distributed locking.

//20. Round Robin Load Balancing
// Round Robin Load Balancing is a simple and widely used load balancing algorithm that distributes incoming requests evenly across a group of servers.
...
// 21. Sticky Sessions
// Sticky Sessions, also known as session affinity, is a load balancing technique that ensures that a user's requests are consistently directed to the same server during a session.
// This is particularly useful for applications that maintain user-specific state information on the server side, such as shopping carts or user preferences.
// Sticky sessions can be implemented using cookies or URL parameters to track the user's session and route subsequent requests to the same server.
// While sticky sessions can improve user experience by maintaining state, they can also lead to uneven load distribution and potential
// bottlenecks if certain servers become overloaded with requests from many users.

// 22. AWS CloudWatch
// AWS CloudWatch is a monitoring and observability service provided by Amazon Web Services (AWS) that allows you to collect, monitor, and analyze metrics, logs, and events from your AWS resources 
// and applications. CloudWatch provides real-time visibility into the performance and health of your applications, enabling you to set alarms,
...
// 23. Celebrity Problem
// The Celebrity Problem is a classic problem in computer science and graph theory that involves identifying a "celebrity" in a group of people.
// A celebrity is defined as someone who is known by everyone else but does not know anyone else in the group.
// The problem can be represented as a directed graph, where each person is a node, and an edge from person A to person B indicates that A knows B.
// The goal is to find the celebrity (if one exists) using the least number of comparisons or queries about who knows whom.
// A common approach to solve the Celebrity Problem is to use a two-pointer technique or a stack-based approach to eliminate non-celebrities and identify the potential celebrity.
// The problem has applications in social networks, recommendation systems, and other scenarios where relationships between entities need to be analyzed.

// 24. Cross Shard Join
// Cross Shard Join is a technique used in distributed databases to perform join operations across multiple shards.
// In a sharded database, data is partitioned into different shards based on a sharding key, and each shard may reside on a different server or node.
...
// 25. Hotspot Problem
// The Hotspot Problem occurs in distributed systems when a particular resource or node becomes a bottleneck due to high demand or traffic.
...
### Fintech example
For a payment transaction, you need to consider how quickly replicas receive the confirmed transaction. You should not assume that an asynchronously replicated read will always immediately reflect the latest write.
